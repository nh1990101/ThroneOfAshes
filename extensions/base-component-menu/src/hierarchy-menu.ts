...
function getBaseComponentMenuItems() {
    ...
    try {
        ...
        const tsFiles = files.filter((f: string) => f.endsWith('.ts') && !f.endsWith('.meta'));

        const menuItems = tsFiles.map((file: string) => {
...
async function createComponentNode(componentName: string) {
    try {
        // 获取当前选中的节点
